...
class Arguments(argtyped.Arguments):
    ...
    show_progress: Switch = (
        False  # show a progress bar for each worker process; large overhead
    )
    force_reprocess: Switch = (
        False  # also process repos that are recorded as processed in DB
    )
        ...
        for path, sha in zip(makefile["binaries"], makefile["sha256"]):
            ...
            for line_num, j in enumerate(decompiled_json):
                ...
                if func_name.startswith("_"):
                    ...
                    decompiled_code = (
                        re.sub(  # replace all identifiers with matching name
